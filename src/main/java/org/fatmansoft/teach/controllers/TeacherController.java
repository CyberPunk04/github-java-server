...
public class TeacherController {
    //Java 对象的注入 我们定义的这下Java的操作对象都不能自己管理是由有Spring框架来管理的， TeacherController 中要使用StudentRepository接口的实现类对象，
...
//        p.setName(CommonMethod.getString(form,"name"));
        ...
        return CommonMethod.getReturnData(t.getTeacherId());  // 将teacherId返回前端
    }
     ...
     * @return
     */
    public List getTeacherScoreList(List<Score>sList){
   ...
   /* *//**
    ...
    public DataResponse getTeacherIntroduceData(@Valid @RequestBody DataRequest dataRequest) {
        ...
        User u = uOp.get();
        Optional<Teacher> sOp= teacherRepository.findByPersonPersonId(u.getPerson().getPersonId());  // 查询获得 Student对象
        if(!sOp.isPresent())
            return CommonMethod.getReturnMessageError("该教师不存在！");
        Teacher t= sOp.get();
        Map info = getMapFromTeacher(t);  // 查询学生信息Map对象
        List<Score> sList = scoreRepository.findByStudentStudentId(t.getStudentId()); //获得学生成绩对象集合
        Map data = new HashMap();
        data.put("info",info);
        data.put("scoreList",getStudentScoreList(sList));
        data.put("markList",getStudentMarkList(sList));
        data.put("feeList",getStudentFeeList(t.getStudentId()));
...
*/
    ...
    public ResponseEntity<StreamingResponseBody> getTeacherIntroducePdf(@Valid @RequestBody DataRequest dataRequest) {
        ...
        Map info = getMapFromTeacher(t);  //获取教师信息
